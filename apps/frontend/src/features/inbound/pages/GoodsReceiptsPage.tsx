...
export default function GoodsReceiptsPage() {
  ...
  const [printOrder, setPrintOrder] = React.useState<any | null>(null);
    ...
    <div className="space-y-4">
              ...
              <div className="w-[420px] shrink-0 border-l border-slate-200 bg-slate-50 overflow-y-auto flex flex-col">
                ...
                <div className="border-t border-slate-200 p-6 bg-white shrink-0 flex flex-col gap-3">
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setPrintOrder(selectedOrder)} className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl border-2 border-slate-200 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-60">
      ...
      {printOrder && (
        <GoodsReceiptModal
          ...
          onConfirm={() => {}}
          items={(printOrder.details || []).map((d: any) => ({
            product: {
              id: d.product?.id || '',
            ...
            qty: Number(d.actualQty || d.requestedQty || 0),
            timestamp: new Date()
          }))}
          viewMode={true}
          receiptNo={printOrder.orderCode}
          receiptDate={`Ngày ${new Date(printOrder.createdAt || Date.now()).getDate().toString().padStart(2, '0')} Tháng ${(new Date(printOrder.createdAt || Date.now()).getMonth() + 1).toString().padStart(2, '0')} Năm ${new Date(printOrder.createdAt || Date.now()).getFullYear()}`}
          supplierName={printOrder.sourcePurchaseOrder?.supplier?.name || 'Khách lẻ / Chưa xác định'}
        />
      )}
    </div>
  );
