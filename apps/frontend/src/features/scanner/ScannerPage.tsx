...
} from "../offline-sync/db/indexedDb";
import {
  QrCode,
  Camera,
  ArrowDownToLine,
...
export default function ScannerPage() {
      ...
      setScannedItems((prev) => {
        const existing = prev.findIndex(
          (item) =>
            ...
            (product.internalSku &&
              item.product.internalSku === product.internalSku),
        );
        if (existing >= 0) {
          const updated = [...prev];
          ...
          showToast(
            "success",
            `Đã tăng số lượng "${product.name}": ${nextQty}`,
          );
          return updated;
        }
  ...
  const syncOfflineReceipts = useCallback(async () => {
    try {
      ...
      if (successCount > 0) {
        showToast(
          "success",
          `Đã tự động đồng bộ ${successCount} phiếu lưu ngoại tuyến lên hệ thống!`,
        );
      }
    } catch (err) {
  ...
  const submitInbound = async () => {
    ...
    setSubmitting(true);
    try {
      const firstSupplier = scannedItems.find((item) => item.product.supplier)
        ?.product.supplier;
      ...
      if (!navigator.onLine) {
        ...
        showToast(
          "success",
          "Đang ngoại tuyến. Phiếu nhập kho đã được lưu cục bộ và sẽ tự động đồng bộ khi có kết nối.",
        );
        setScannedItems([]);
      ...
      showToast(
        "success",
        `Đã tạo phiếu nhập kho với ${scannedItems.length} sản phẩm`,
      );
      setScannedItems([]);
      setShowReceiptModal(false);
    } catch (err: any) {
  ...
  const submitOutbound = async () => {
    ...
    try {
      if (!navigator.onLine) {
        ...
        showToast(
          "success",
          "Đang ngoại tuyến. Phiếu xuất kho đã được lưu cục bộ và sẽ tự động đồng bộ khi có kết nối.",
        );
        setScannedItems([]);
        return;
      }
      ...
      showToast(
        "success",
        `Đã tạo phiếu xuất kho với ${scannedItems.length} sản phẩm`,
      );
      setScannedItems([]);
    } catch (err: any) {
  ...
  const submitStocktake = async () => {
    ...
    try {
      if (!navigator.onLine) {
        ...
        showToast(
          "success",
          "Đang ngoại tuyến. Phiếu kiểm kê đã được lưu cục bộ và sẽ tự động đồng bộ khi có kết nối.",
        );
        setScannedItems([]);
        return;
      }
      ...
      showToast(
        "success",
        `Đã tạo phiên kiểm kê với ${scannedItems.length} sản phẩm`,
      );
      setScannedItems([]);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 sm:gap-2.5 rounded-xl border-2 border-cyan-500 bg-cyan-600 px-3.5 sm:px-4 py-2 text-white shadow-md">
            <QrCode className="h-5 w-5 text-cyan-100 flex-shrink-0" />
            <h1 className="text-base sm:text-lg font-bold tracking-tight text-white">
              Trạm Quét Mã Vạch / QR Code
            </h1>
          </div>
        </div>

        {/* MODE SELECTOR */}
        <div className="flex flex-wrap sm:inline-flex w-full sm:w-auto rounded-2xl border-2 border-slate-200 bg-white p-1 shadow-sm">
          {(Object.keys(modeConfigs) as ScanMode[]).map((m) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setMode(m)}
                className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 sm:gap-2 rounded-xl px-2.5 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold transition-all cursor-pointer ${
                  isActive
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl border-2 border-cyan-500 bg-white p-5 sm:p-8 text-center shadow-lg">
        <div className="mx-auto flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-2xl sm:rounded-3xl border-2 border-cyan-200 bg-cyan-50 text-cyan-600 shadow-inner">
          <ScanLine className="h-8 w-8 sm:h-10 sm:w-10 animate-pulse" />
        </div>

        <h2 className="mt-3 sm:mt-4 text-lg sm:text-xl font-black text-slate-900">
          Chế độ hiện tại:{" "}
          <span className="text-cyan-600">{modeConfigs[mode].label}</span>
        </h2>
        <p className="mt-1 text-xs sm:text-sm font-medium text-slate-500">
          {modeConfigs[mode].desc}
        </p>

        <div className="mt-5 sm:mt-6 flex flex-wrap items-center justify-center gap-4">
          <ScanBarcodeButton
            onClick={() => setScannerOpen(true)}
            label="Bắt đầu quét Camera"
          />
        </div>

        <p className="mt-3 sm:mt-4 text-[11px] sm:text-xs font-semibold text-slate-400">
          Định dạng hỗ trợ: QR Code, EAN-13, Code-128, UPC-A, Data Matrix và các
          mã vạch nhà cung cấp
        </p>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
              ...
              Danh Sách Đã Quét ({scannedItems.length} Mặt hàng · {totalItems}{" "}
              Đơn vị)
            </h2>
            <button
              type="button"
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span
                      className={`rounded-lg border px-2 py-0.5 text-xs font-bold ${modeConfigs[mode].badgeClass}`}
                    >
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[10001] flex items-center gap-2.5 rounded-2xl px-6 py-3.5 font-bold text-white shadow-2xl animate-in fade-in slide-in-from-bottom-5 border-2 ${
            toast.type === "success"
