...
export default function InventoryBaseUnitReportPage() {
  ...
  const [columnVis, setColumnVis] = useState({
    ...
    unitPrice: true,
    totalValue: true,
    pendingExportQty: true,
    pendingOrderQty: true,
  });
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const res = await reportsApi.getInventorySummaryReport(startDate, endDate, 'all', 'base_unit');
      setData(Array.isArray(res) ? res : []);
    } catch (err: any) {
      setError(err?.message || 'Không thể kết nối dữ liệu báo cáo hàng tồn đơn vị gốc');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredGroups = useMemo(() => {
    ...
    return data
      ...
      .filter(Boolean) as InventoryGroup[];
  }, [data, searchTerm]);
  ...
  const totals = useMemo(() => {
    ...
    filteredGroups.forEach((g) => {
      (g.items || []).forEach((i) => {
        init += i.initialStock || 0;
        inQty += i.importQty || 0;
        outQty += i.exportQty || 0;
        finalQty += i.finalStock || 0;
        val += i.totalValue || 0;
      });
    });

    return { init, inQty, outQty, finalQty, val };
  }, [filteredGroups]);

  const handleExportExcel = () => {
    if (allItems.length === 0) return;
    const headers = ['STT', 'Mã SKU', 'Tên sản phẩm', 'Tồn đầu', 'Nhập', 'Xuất', 'Tồn cuối', 'Đơn giá', 'Tổng giá trị tồn'];
    const rows: any[] = [];
    let idx = 1;
    filteredGroups.forEach((g) => {
      (g.items || []).forEach((i) => {
    ...
    const summaryRow = ['Tổng cộng', '', '', totals.init, totals.inQty, totals.outQty, totals.finalQty, '', totals.val];
    const csvContent = '\uFEFF' + [headers.join(','), ...rows.map((r) => r.join(',')), summaryRow.join(',')].join('\n');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `Bao_Cao_Ton_Kho_Don_Vi_Goc_${startDate}_den_${endDate}.csv`);
  ...
  return (
    <div className="space-y-4 pb-12 animate-in fade-in duration-200">
      {/* ═══ TOP HEADER SECTION - CYAN ONLY FOR TITLE BADGE ═══ */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center gap-2.5 rounded-2xl bg-cyan-600 px-5 py-2.5 text-white shadow-md">
            ...
            <h1 className="text-xl font-extrabold tracking-tight uppercase">BÁO CÁO HÀNG TỒN THEO ĐƠN VỊ GỐC</h1>
          </div>
        </div>
      ...
      {/* ═══ FILTER & SEARCH PANEL - CLEAN WHITE ═══ */}
      ...
      {/* ═══ DATA TABLE - CYAN THEME HEADER & SLATE GROUPS ═══ */}
            ...
            <tbody className="divide-y divide-slate-200 bg-white text-xs sm:text-sm font-medium text-slate-800">
              {loading ? (
                ...
                filteredGroups.map((group) => {
                  ...
                  const grpInit = items.reduce((s, i) => s + (i.initialStock || 0), 0);
                  const grpIn = items.reduce((s, i) => s + (i.importQty || 0), 0);
                  const grpOut = items.reduce((s, i) => s + (i.exportQty || 0), 0);
                  const grpFinal = items.reduce((s, i) => s + (i.finalStock || 0), 0);
                  const grpValue = items.reduce((s, i) => s + (i.totalValue || 0), 0);
                    ...
                    <React.Fragment key={group.groupName || 'group'}>
                      ...
                      {items.map((row, idx) => (
                        <tr key={row.id || idx} className="hover:bg-slate-50 transition group">
                          <td className="py-3 px-3 text-center border-r border-slate-200 font-normal text-slate-900">{idx + 1}</td>
                          {columnVis.sku && <td className="py-3 px-3 text-left border-r border-slate-200 font-mono font-normal text-slate-900">{row.sku}</td>}
                          {columnVis.name && <td className="py-3 px-4 text-left border-r border-slate-200 font-normal text-slate-900">{row.name}</td>}
                          {columnVis.initialStock && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{row.initialStock}</td>}
                          {columnVis.importQty && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{row.importQty}</td>}
                          {columnVis.exportQty && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{row.exportQty}</td>}
                          {columnVis.finalStock && (
                            <td className={`py-3 px-3 text-right border-r border-slate-200 font-normal ${row.finalStock < 0 ? 'text-rose-600 font-semibold' : 'text-slate-900'}`}>
                              {row.finalStock}
                            </td>
                          )}
                          {columnVis.unitPrice && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{fmt(row.unitPrice)} đ</td>}
                          {columnVis.totalValue && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{fmt(row.totalValue)} đ</td>}
                          {columnVis.pendingExportQty && <td className="py-3 px-3 text-right border-r border-slate-200 font-normal text-slate-900">{row.pendingExportQty}</td>}
                          {columnVis.pendingOrderQty && <td className="py-3 px-3 text-right font-normal text-slate-900">{row.pendingOrderQty}</td>}
                        </tr>
                      ))}
                      <tr className="bg-slate-100 font-bold text-slate-900 border-b-2 border-slate-300">
                        <td colSpan={3} className="py-2.5 px-4 text-left border-r border-slate-300 uppercase font-bold">Cộng nhóm:</td>
                        {columnVis.initialStock && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">{grpInit}</td>}
                        {columnVis.importQty && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">{grpIn}</td>}
                        {columnVis.exportQty && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">{grpOut}</td>}
                        {columnVis.finalStock && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">{grpFinal}</td>}
                        {columnVis.unitPrice && <td className="py-2.5 px-3 border-r border-slate-300 font-bold">-</td>}
                        {columnVis.totalValue && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">{fmt(grpValue)} đ</td>}
                        {columnVis.pendingExportQty && <td className="py-2.5 px-3 text-right border-r border-slate-300 font-bold">0</td>}
                ...
                <tr>
                  <td colSpan={11} className="py-12 text-center text-slate-400 font-bold">
                    Không tìm thấy dữ liệu hàng tồn theo đơn vị gốc
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showColumnSettings && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs animate-in fade-in">
          <div className="w-full max-w-md rounded-2xl border-2 border-slate-300 bg-white p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2 uppercase">
            ...
            <div className="space-y-2 text-xs font-bold text-slate-700">
              {Object.entries({
                sku: 'Mã SKU',
                name: 'Tên sản phẩm',
                initialStock: 'Tồn đầu',
                importQty: 'Nhập',
                exportQty: 'Xuất',
                finalStock: 'Tồn cuối',
                unitPrice: 'Giá vốn',
                totalValue: 'Giá trị tồn',
                pendingExportQty: 'Chưa giao',
                pendingOrderQty: 'Đang đặt',
              }).map(([key, label]) => (
            ...
            <div className="pt-2 text-right">
              <button
                type="button"
                onClick={() => setShowColumnSettings(false)}
                className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-extrabold text-xs transition cursor-pointer"
              >
                Hoàn tất
              </button>
            </div>
