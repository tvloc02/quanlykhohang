                ...
                {isWarehouseDropdownOpen && (
                  <div className="absolute top-full left-0 mt-2 w-full min-w-[280px] rounded-2xl border-2 border-cyan-500 bg-white p-2 shadow-2xl z-50 animate-in fade-in zoom-in-95">
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition flex items-center justify-between cursor-pointer mb-1 ${selectedBranch === 'ALL'
                    ...
                    {warehouses.length > 0 ? (
                      warehouses.map((wh) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition flex items-center justify-between cursor-pointer mb-1 ${selectedBranch === wh.name
                              ? 'bg-cyan-600 text-white font-extrabold shadow-sm'
                              : 'text-slate-700 hover:bg-cyan-50 hover:text-cyan-800'
                            }`}
                        >
                      ...
                      ['Kho Chi Nhánh HCM', 'Kho Thanh Trì', 'Kho Hà Đông'].map((bName) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition flex items-center justify-between cursor-pointer mb-1 ${selectedBranch === bName
                              ? 'bg-cyan-600 text-white font-extrabold shadow-sm'
                              : 'text-slate-700 hover:bg-cyan-50 hover:text-cyan-800'
                            }`}
                        >
                    ...
                    <tr key={row.id || idx} className="hover:bg-slate-50 transition">
                      ...
                      {validKeys.map((key, colIdx) => {
                        ...
                        return (
                          <td
                            key={key}
                            className={`px-4 py-3.5 border-r border-slate-200 whitespace-nowrap ${isCenterCol
                                ? 'text-center'
                                : isNum
                  ...
                  <tr>
                    ...
                    {validKeys.map((key, colIdx) => {
                      ...
                      return (
                        <td
                          key={key}
                          className={`px-4 py-4 border-r border-slate-300 whitespace-nowrap ${hasSum
