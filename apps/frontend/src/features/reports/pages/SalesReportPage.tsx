    ...
    try {
      ...
      let mappedItems = items.map((item: any, idx: number) => {
        ...
        return {
          ...
          dateOrName:
            item.dateOrName || item.groupName || item.date || `Nhóm ${idx + 1}`,
          salesOrderCount: Number(
          ...
          if (Array.isArray(localList) && localList.length > 0) {
            ...
            mappedItems.forEach((grp) => {
              (grp.orders || []).forEach((o: any) => {
                if (o.orderNo)
                  existingOrderNos.add(String(o.orderNo).trim().toUpperCase());
              });
            });

            localList.forEach((lo) => {
              const oNo = String(lo.orderNo || lo.orderCode || "")
                .trim()
                .toUpperCase();
              const oType = String(lo.orderType || "").toLowerCase();
              const isDisposal = oType === "disposal" || oNo.startsWith("XH");
              const isCancelled = ["ĐÃ HỦY", "CANCELLED", "HỦY"].includes(
                String(lo.status || "").toUpperCase(),
              );

              if (
                isDisposal ||
                isCancelled ||
                (oNo && existingOrderNos.has(oNo))
              ) {
                return;
              }

              const oDateStr = parseAnyDateToLocalString(
                lo.orderDate || lo.createdAt,
              );
              ...
              if (activeGroup === "month") {
                groupKey = oDateStr
                  ? oDateStr.substring(0, 7)
                  : "Không xác định";
                groupLabel = oDateStr
                  ? `Tháng ${oDateStr.substring(5, 7)}/${oDateStr.substring(0, 4)}`
                  : "Không xác định";
              } else if (activeGroup === "year") {
                groupKey = oDateStr
                  ? oDateStr.substring(0, 4)
                  : "Không xác định";
                groupLabel = oDateStr
                  ? `Năm ${oDateStr.substring(0, 4)}`
                  : "Không xác định";
                ...
                groupKey =
                  (lo.customerName || lo.customer || "").trim() ||
                  "Khách lẻ / vãng lai";
                groupLabel = groupKey;
              } else if (activeGroup === "branch") {
                groupKey = (lo.branchCode || lo.warehouseCode || "KHO-TONG")
                  .trim()
                  .toUpperCase();
                groupLabel = groupKey;
              }
              ...
              let foundGroup = mappedItems.find(
                (g) => g.id === groupKey || g.dateOrName === groupLabel,
              );
              if (!foundGroup) {
                foundGroup = {
                  id: groupKey,
              ...
              foundGroup.orders.push({
                ...
                customerName:
                  lo.customerName || lo.customer || "Khách hàng bán lẻ",
