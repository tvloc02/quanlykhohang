...
export default function Login() {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (window.google?.accounts?.id) {
      try {
        console.debug('Triggering google.accounts.id.prompt()');
        window.google.accounts.id.prompt();
      } catch (e) {
        console.error('Error calling prompt()', e);
        setToast({ type: 'error', title: 'Lỗi Google', message: 'Không thể mở popup Google' });
      }
    } else {
      console.warn('google.accounts.id not found on window');
  ...
  React.useEffect(() => {
    ...
    const initializeGoogle = () => {
      ...
      // Render official Google button into hidden container so library handlers are wired
      try {
        if (hiddenGoogleRef.current && window.google?.accounts?.id?.renderButton) {
          window.google.accounts.id.renderButton(hiddenGoogleRef.current, { theme: 'outline', size: 'large' });
          // make the official button visible for testing
          Object.assign(hiddenGoogleRef.current.style, {
            display: 'block',
            marginTop: '8px',
            width: '100%',
          });
          console.debug('Rendered official Google button (visible for testing)');
        }
      } catch (e) {
        ...
        <div className="w-full lg:w-[53%] p-10 lg:p-20 flex flex-col relative bg-white">
          <div className="max-w-[500px] w-full mx-auto flex-1 flex flex-col justify-center">
            ...
            <div className="w-full flex items-center justify-center">
              {/* Official Google button container (rendered by Google's library) */}
              <div ref={hiddenGoogleRef as any} className="w-full max-w-md" />
            </div>
